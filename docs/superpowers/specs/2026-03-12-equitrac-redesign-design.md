...
Note: The Install DRC toggle lives here because it originates from the `enroll_drc` flag in the enrollment command. Its dependent settings (DRC System Name Mode, etc.) appear in Step 2: Preferences, matching the EQPrinterUtilityX Preferences dialog layout. The DRC System Name card in Step 2 is only shown when Install DRC is enabled.

## Step 2: Preferences (EQPrinterUtilityX Settings)

Mirrors the EQPrinterUtilityX Preferences dialog, using styled toggles instead of native checkboxes. CAS Server appears here because it is a field in the Preferences dialog (not part of the enrollment command).
...
| Interface | dropdown (any, en0, en1, en2, en3, en4, en5) | `IP_ADDR_INTERFACE` | Visible when mode is IP Address or DNS Hostname. Default "any" (empty string in config). |
...
| Field | Type | Required | Config Key |
|---|---|---|---|
| DRE Server | text | yes (if any DRE printers exist; validation skipped if only IP printers) | `DRE_SERVER` |

Note: `DRE_SERVER` is always written to the config profile (empty string if not set). The postinstall script already handles this with `defaults read ... 2>/dev/null`.
...
| Port | text | Visible when Raw. Default 9100. Required when Raw. |
| Queue | text | Visible when LPR. Required when LPR. |
| PPD | text | Default "Generic" |
...
- **Step 1 (Enrollment)**: Complete when all required Credentials fields are filled
- **Step 2 (Preferences)**: Complete when CAS Server is filled (all other fields have defaults)
- **Step 3 (Printers)**: Always completable (printers are optional; if DRE printers exist, DRE Server must be filled)
- **Step 4 (Packages)**: Complete when Installer PKG filename is filled
- **Step 5 (Build)**: Complete when build script is generated
- **Step 6 (Deploy)**: Terminal step

### Forward Navigation
Steps must be visited in order (cannot skip ahead). Clicking a completed step returns to it. The Next button validates required fields for the current step before advancing.

## Design Notes

### Feature Bitmask vs Standalone Prefs
The 5 feature toggles (Client Billing, Release Key, etc.) contribute to the `Feature Selection` bitmask in EquitracOfficePrefs. The new Preferences fields (Cache Login, Prompt for Password, User ID Label, etc.) are standalone key-value pairs written directly to EquitracOfficePrefs — they are NOT part of the bitmask.

### Backward Compatibility
New config keys must be read with fallback defaults in the postinstall script (using `|| echo "default"` pattern). Profiles generated by the old tool version (missing new keys) must still work — the script falls back to the same defaults as today's hardcoded values.

## Postinstall Script Updates

The `scripts/equitrac_postinstall.sh` must be updated to:

1. **Read new config keys**: `SKIP_LINK_LOCAL_IP`, `IP_ADDR_INTERFACE`, `REG_MACHINE_ID_DNS`, `USE_CACHED_LOGIN`, `PROMPT_FOR_PASSWORD`, `USER_ID_LABEL`, `IGNORE_SUPPLIES_LEVEL_JOB`. Existing keys (`CAS_SERVER`, `DRE_SERVER`, `INSTALL_DRC`, `DRC_SYS_NAME_MODE`, etc.) remain unchanged.
